 ...
 * token, opens a WAL-mode SQLite store on the WSL-native filesystem.
 * Port of `daemon/src/main.rs`.
 *
 * Task 2 (this file, initial cut): scaffold + `/health` only. Task 3 adds
 * the auth preHandler + read routes via `registerRoutes` and the startup
 * rehydration log.
 */
 ...
 * data, plain-text `ok`) and this daemon emits no permissive CORS headers on
 * any route (FND-05). Every other route (Task 3) requires the per-install
 * token.
 */
...
async function main(): Promise<void> {
  ...
  void token; // wired into the auth preHandler in Task 3

  const dbPath = join(dir, "cockpit.db");
  openDb(dbPath); // opens/creates schema, verifies WAL — no queries yet (Task 3)
