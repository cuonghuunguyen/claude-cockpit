...
describe("ingest routes (real Fastify instance + real on-disk SQLite temp file)", () => {
  ...
  it("full hook lifecycle drives the expected status/timeline sequence", async () => {
    ...
    // "Bash" no longer holds on PreToolUse (D-14/03-05): general tool-call
    // permission gating moved to the wildcard PermissionRequest hook, so
    // this is now a fast, non-blocking ack -- no hold, no decision to await.
    const preToolUseRes = await auth(supertest(app.server).post("/hooks/pre-tool-use")).send({
      session_id: "life1",
      tool_name: "Bash",
      tool_input: { command: "ls" },
    });
    expect(preToolUseRes.status).toBe(200);
